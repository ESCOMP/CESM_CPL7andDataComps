...
contains
  ...
  subroutine cime_pre_init1(esmf_log_option)
    use init_pio_mod, only: init_pio_init1, init_pio_init2
    ...
    call init_pio_init1(num_inst_total,NLFileName, driver_comm)
    !
    ! If pio_async_interface is true Global_comm is MPI_COMM_NULL on the servernodes
    ! and server nodes do not return from init_pio_init2
    !
   ...
   do eei = 1,num_inst_esp
    ...
    !  init_pio_init2
    !
    call init_pio_init2(comp_id,comp_name,comp_iamin,comp_comm,comp_comm_iam)
...
113 format( A14, 999999999(:, ',', f13.3) )
  ...
  subroutine cime_final()
    ...
    use init_pio_mod, only : init_pio_finalize
